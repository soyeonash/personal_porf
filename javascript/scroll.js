jQuery(function ($) {
  ...
  $(".site-primary").impulse({ effect: "easeOutBack", fluid: false });
});
...
(function ($, nib) {
  $.fn.impulse = function (options) {
    ...
    function onFringe(matter, cue, genesis) {
      var put = Math.round(genesis || $(matter).scrollTop()),
        ...
        below =
          destined[cue] > brink[cue] && put == brink[cue] && fad.orbit > 0;
      return above || below;
    }
  ...
  $.fn.evoke = function (unit) {
    var lot = $.Event("wheel.excite", { mien: true }),
      bulk;
    return this.each(function () {
      if ($.zenith(this)) {
        if (!bulk) {
...
(function ($) {
  ...
  $.extend(b, {
    ...
    Bounce: function (p) {
      ...
      return (
        1 / Math.pow(4, 3 - h) - 7.5625 * Math.pow((f * 3 - 2) / 22 - p, 2)
      );
    }
  });
  $.each(b, function (n, e) {
